...
#### 9.1차 완성본에 대한 기능적/비기능적 요구사항을 커버하는지 검증
 ...
 (O)  1. 관리자는 정수기 정보를 등록한다.
 (O)  2. 고객은 관리자가 등록한 정수기를 조회한 후 원하는 정수기를 렌탈 신청한다.
 (O)  3. 고객은 신청한 정수기의 렌탈비를 결제한다
 (O)  4. 렌탈 신청이 완료되면 정수기를 배송 시작한다
 (O)  5. 고객은 렌탈 신청을 취소할 수 있다.
 (O)  6. 렌탈 신청이 취소되면 결제가 취소된다.
 (O)  7. 결제 취소되면 배송이 취소된다.
 (O)  8. 배송 시작/취소되면 정수기 재고 수량을 조정한다. 
 (O)  9. 고객은 언제든지 렌탈/배송 현황을 조회한다. (CQRS-View)
 (O) 10. 렌탈 신청 상태,배송상태가 바뀔때마다 해당 고객에게 문자 메세지가 발송된다. 
